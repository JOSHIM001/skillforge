...
from middleware.security import request_id_middleware
...
async def lifespan(app: FastAPI):
    ...
    if not await ping_redis():
        logger.error("Redis unreachable on startup")
    await init_db()
...
def create_app() -> FastAPI:
    ...
    # Request ID tracing — attaches unique ID to every request for log tracing
    ...
    async def health_check():
        ...
        return {
            ...
            "ai":       "configured" if groq_ok else "missing_key",
            "version":  "1.0.0",
            "env":      settings.environment,
        }

    # Global error handler — returns request_id so users can report bugs
    @app.exception_handler(Exception)
    async def unhandled_exception_handler(request: Request, exc: Exception):
        request_id = getattr(request.state, "request_id", "unknown")
        logger.exception("Unhandled exception", path=request.url.path, request_id=request_id)
        return JSONResponse(
            status_code=500,
